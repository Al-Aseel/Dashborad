...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Error in v1/user/reset-password API route:", error);
    return NextResponse.redirect(
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          status: "error",
          message:
            responseData.message || "حدث خطأ أثناء إعادة تعيين كلمة المرور",
        },
    ...
    console.error("Error in v1/user/reset-password POST:", error);
    return NextResponse.json(
      {
        status: "error",
